...
var Palette = (function () {
  ...
  Palette.prototype.addToPrevDish = function (blobs, new_id) {
    ...
    for (var i = 0; i < blobs.length; i++) {
      var blob_copy = new Blob(blobs[i].x, blobs[i].y, blobs[i].color);
  ...
  Palette.prototype.subFromPrevDish = function (blob) {
    ...
    var blobs = [];
    var dish_wo_blob = new Dish(++id, prevDish, blobs, [], []);
    for (var i = 0; i < prevDish.blobs.length; i++) {
      if (!_this.isSameBlob(prevDish.blobs[i], blob)) {
        var prevBlob = new Blob(prevDish.blobs[i].x, prevDish.blobs[i].y, prevDish.blobs[i].color);
  ...
  Palette.prototype.addBlob = function (event, color) {
    ...
    var newX = x - newLeft;
    var newY = y - newTop;
    var blob = new Blob((x - newLeft) * this.pixelRatio - 15,
    ...
    if (nearestBlob !== null) {
      ...
      if (_this.isDifferent(blob, nearestDish)) {
        ...
        var newDish = new Dish(id, nearestDish, updated_blobs, [], []);
        newDish.swatches = [];
        newDish.swatches.push(new Color(curr_r, curr_g, curr_b));
        ...
        nearestDish.swatches.push(new Color(curr_r, curr_g, curr_b));
      }
    } else {
      ...
      blob.dish_id = id;
      blobs.push(blob);
      var dish = new Dish(id, null, blobs, [], []);
      dish.blobCoords_x.push(x);
      dish.blobCoords_y.push(y);
      ...
      dish.swatches.push(new Color(curr_r, curr_g, curr_b));
  ...
  Palette.prototype.dragBlob = function (event, blob) {
    ...
    var new_blob = new Blob(blob.x, blob.y, blob.color);
    ...
    if (nearestBlob !== null) {
      ...
      if (_this.isDifferent(blob, nearestDish)) {
        ...
        if (blob.wasGrouped) {
          ...
          var blobs = [];
          blobs.push(new_blob);
          var new_dish = new Dish(id, prev_dish, blobs, [], []);
      ...
      Palette.prototype._initEventHandle = function () {
        ...
        parent.addEventListener('mouseup', function (event) {
          ...
          if (_this.isMouseDown && deleteBlob === true) {
            ...
            _this.addBlob(event, new Color(curr_r, curr_g, curr_b));  // add color here
              ...
              if (!alreadyClickedMarker) {
                for (var i = 0; i < _this.blobs.length; i++) {
                  if (_this.isSameBlob(_this.blobs[i], _this.activeBlob)) {
                    blob = new Blob(_this.activeBlob.x, _this.activeBlob.y, new Color(curr_r, curr_g, curr_b));
    ...
    var Renderer = (function () {
      ...
      Renderer.prototype.render = function (palette) {
        ...
        var palette = new Palette(mixingCanvas);
